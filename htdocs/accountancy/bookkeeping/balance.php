...
if ($type == 'sub') {
	$context_default = 'balancebysubaccountlist';
} else {
...
$title_page = '';
if ($type == 'sub') {
	$title_page .= $langs->trans("SubAccountBalance");
} else {
...
if ($action != 'export_csv') {
	...
	if (empty($conf->global->MAIN_DISABLE_FULL_SCANLIST)) {
		...
		$nbtotalofrecords = $object->fetchAllBalance($sortorder, $sortfield, 0, 0, $filter, 'AND', $showsubaccount);
		if ($nbtotalofrecords < 0) {
			setEventMessages($object->error, $object->errors, 'errors');
		}
	}

	$result = $object->fetchAllBalance($sortorder, $sortfield, $limit, $offset, $filter, 'AND', $showsubaccount);
	if ($result < 0) {
		setEventMessages($object->error, $object->errors, 'errors');
	}
	...
	if (empty($reshook)) {
		...
		$newcardbutton = '';
		if ($type == 'sub') {
			$newcardbutton .= dolGetButtonTitle($langs->trans('GroupByAccountAccounting'), '', 'fa fa-stream paddingleft imgforviewmode', DOL_URL_ROOT . '/accountancy/bookkeeping/balance.php?' . $url_param, '', 1, array('morecss' => 'marginleftonly'));
			$newcardbutton .= dolGetButtonTitle($langs->trans('GroupBySubAccountAccounting'), '', 'fa fa-align-left vmirror paddingleft imgforviewmode', DOL_URL_ROOT . '/accountancy/bookkeeping/balance.php?type=sub&' . $url_param, '', 1, array('morecss' => 'marginleftonly btnTitleSelected'));
		} else {
			$newcardbutton .= dolGetButtonTitle($langs->trans('GroupByAccountAccounting'), '', 'fa fa-stream paddingleft imgforviewmode', DOL_URL_ROOT . '/accountancy/bookkeeping/balance.php?' . $url_param, '', 1, array('morecss' => 'marginleftonly btnTitleSelected'));
			$newcardbutton .= dolGetButtonTitle($langs->trans('GroupBySubAccountAccounting'), '', 'fa fa-align-left vmirror paddingleft imgforviewmode', DOL_URL_ROOT . '/accountancy/bookkeeping/balance.php?type=sub&' . $url_param, '', 1, array('morecss' => 'marginleftonly'));
		}
	}

	print_barre_liste($title_page, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $button, $result, $nbtotalofrecords, 'title_accountancy', 0, $newcardbutton, '', $limit);
	...
	if ($type == 'sub') {
		print $formaccounting->select_auxaccount($search_accountancy_code_start, 'search_accountancy_code_start', $langs->trans('From'), 'maxwidth200');
	} else {
	...
	print ' ';
	if ($type == 'sub') {
		print $formaccounting->select_auxaccount($search_accountancy_code_end, 'search_accountancy_code_end', $langs->trans('to'), 'maxwidth200');
	} else {
	...
	foreach ($object->lines as $line) {
		...
		$accountingaccountstatic->fetch(null, $line->{$field}, true);
		...
		$opening_balance = isset($opening_balances["'".$line->{$field}."'"]) ? $opening_balances["'".$line->numero_compte."'"] : 0;
		$total_opening_balance += $opening_balance;

		$tmparrayforrootaccount = $object->getRootAccount($line->{$field});
		...
		if (!empty($show_subgroup)) {
			...
			if (empty($displayed_account) || $root_account_number != $displayed_account) {
				...
				print '<td colspan="'.($colspan+1).'" class="tdforbreak">'.$line->{field}.($root_account_description ? ' - '.$root_account_description : '').'</td>';
				print '</tr>';
		...
		$urlzoom = '';
		if (!empty($line->numero_compte) || !empty($line->subledger_account)) {
			$urlzoom = DOL_URL_ROOT.'/accountancy/bookkeeping/listbyaccount.php?'.($type == "sub" ? 'type=sub&' : '').'search_accountancy_code_start='.urlencode($line->{$field}).'&search_accountancy_code_end='.urlencode($line->{$field});
