...
/**
 *  \file       htdocs/accounting/admin/card.php
 *  \ingroup    Accounting
...
require_once DOL_DOCUMENT_ROOT . '/accounting/class/accountingsystem.class.php';
require_once DOL_DOCUMENT_ROOT . '/core/class/html.formaccounting.class.php';

$error = 0;

// Load translation files required by the page
$langs->loadLangs(array("bills","accounting"));
...
{
	$urltogo=$backtopage?$backtopage:dol_buildpath('/accounting/admin/account.php', 1);
...
if ($action == 'add' && $user->rights->accounting->chartofaccount)
{
	if (! $cancel) {
		...
		dol_syslog('accounting/admin/card.php:: $sql=' . $sql);
		$result = $db->query($sql);
...
elseif ($id > 0 || $ref) {
	...
	if ($result > 0) {
		...
		if ($action == 'update')
		{
			...
			// View mode
			$linkback = '<a href="'.DOL_URL_ROOT.'/accounting/admin/account.php?restore_lastsearch_values=1">' . $langs->trans("BackToList") . '</a>';
