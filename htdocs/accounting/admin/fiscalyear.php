...
if ($result)
{
	...
	$title = $langs->trans('AccountingPeriods');
	print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $params, $sortfield, $sortorder, '', $num, $nbtotalofrecords, 'title_accounting', 0, $addbutton, '', $limit, 1);
	...
	print '<td class="center">' . $langs->trans("NumberOfAccountingEntries") . '</td>';
	print '<td class="center">' . $langs->trans("NumberOfAccountingMovements") . '</td>';
	...
	if ($num) {
		...
		while ( $i < $num && $i < $max ) {
			...
			print '<td class="center">' . $object->getAccountingEntriesByFiscalYear($obj->date_start, $obj->date_end) . '</td>';
			print '<td class="center">' . $object->getAccountingMovementsByFiscalYear($obj->date_start, $obj->date_end) . '</td>';
