...
/**
 * \file		htdocs/accounting/admin/index.php
 * \ingroup		Accounting
...
$langs->loadLangs(array("compta","bills","admin","accounting"));
...
if ($action == 'setenablesubsidiarylist') {
    ...
    $res = dolibarr_set_const($db, "ACCOUNTING_COMBO_FOR_AUX", $setenablesubsidiarylist, 'yesno', 0, '', $conf->entity);
    if (! $res > 0)
        $error ++;
    if (! $error) {
        setEventMessages($langs->trans("SetupSaved"), null, 'mesgs');
...
if (! empty($user->admin))
{
    ...
    print '<tr class="oddeven">';
    print '<td>' . $langs->trans("ACCOUNTING_COMBO_FOR_AUX") . '</td>';
