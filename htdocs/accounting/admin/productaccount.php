...
$langs->loadLangs(array("companies","compta","accounting","products"));
...
if (empty($accounting_product_mode)) $accounting_product_mode='ACCOUNTING_SELL';
		...
		if (! empty($chk_prod)) {
			...
			foreach ($chk_prod as $productid)
			{
				...
				if ($result <= 0) {
					...
					if ($accounting_product_mode == 'ACCOUNTING_BUY') {
						$sql .= " SET accounting_code_buy = " . $accounting->account_number;
					}
					if ($accounting_product_mode == 'ACCOUNTING_SELL') {
						$sql .= " SET accounting_code_sell = " . $accounting->account_number;
					}
...
$sql = "SELECT p.rowid, p.ref, p.label, p.description, p.tosell, p.tobuy, p.accounting_code_sell, p.accounting_code_buy, p.tms, p.fk_product_type as product_type,";
$sql.= " aa.rowid as aaid";
$sql.= " FROM " . MAIN_DB_PREFIX . "product as p";
$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."accounting_account as aa ON";
if ($accounting_product_mode == 'ACCOUNTING_BUY') {
    $sql.=" p.accounting_code_buy = aa.account_number AND aa.fk_pcg_version = '" . $pcgvercode . "'";
}
else
{
    $sql.=" p.accounting_code_sell = aa.account_number AND aa.fk_pcg_version = '" . $pcgvercode . "'";
}
$sql.= ' WHERE p.entity IN ('.getEntity('product').')';
if ($accounting_product_mode == 'ACCOUNTING_BUY') {
    if (strlen(trim($search_current_account))) {
        $sql .= natural_search("p.accounting_code_buy", $search_current_account);
    }
} else {
    if (strlen(trim($search_current_account))) {
        $sql .= natural_search("p.accounting_code_sell", $search_current_account);
    }
}
...
if ($result)
{
    ...
    print load_fiche_titre($langs->trans("ProductsBinding"), '', 'title_accounting');
	print '<br>';

	print $langs->trans("InitAccountingDesc") . '<br>';
	print '<br>';
	...
	print "</tr>\n";
	print '<tr class="oddeven"><td class="titlefield"><input type="radio" name="accounting_product_mode" value="ACCOUNTING_SELL"' . ($accounting_product_mode != 'ACCOUNTING_BUY' ? ' checked' : '') . '> ' . $langs->trans('OptionModeProductSell') . '</td>';
	print '<td>'.$langs->trans('OptionModeProductSellDesc');
	print "</td></tr>\n";
	print '<tr class="oddeven"><td class="titlefield"><input type="radio" name="accounting_product_mode" value="ACCOUNTING_BUY"' . ($accounting_product_mode == 'ACCOUNTING_BUY' ? ' checked' : '') . '> ' . $langs->trans('OptionModeProductBuy') . '</td>';
	...
	if (! empty($conf->global->ACCOUNTING_SHOW_PROD_DESC)) print '<td class="liste_titre"><input type="text" class="flat" size="20" name="search_desc" value="' . dol_escape_htmltag($search_desc) . '"></td>';
	// On sell
	if ($accounting_product_mode == 'ACCOUNTING_SELL') print '<td class="liste_titre"></td>';
	// On buy
	if ($accounting_product_mode == 'ACCOUNTING_BUY') print '<td class="liste_titre"></td>';
	// Current account
    ...
    if (! empty($conf->global->ACCOUNTING_SHOW_PROD_DESC)) print_liste_field_titre("Description", $_SERVER["PHP_SELF"], "p.description", "", $param, '', $sortfield, $sortorder);
    if ($accounting_product_mode == 'ACCOUNTING_SELL') print_liste_field_titre("OnSell", $_SERVER["PHP_SELF"], "p.tosell", "", $param, '', $sortfield, $sortorder, 'center ');
    if ($accounting_product_mode == 'ACCOUNTING_BUY')  print_liste_field_titre("OnBuy", $_SERVER["PHP_SELF"], "p.tobuy", "", $param, '', $sortfield, $sortorder, 'center ');
   	if ($accounting_product_mode == 'ACCOUNTING_BUY') $fieldtosortaccount="p.accounting_code_buy";
   	else $fieldtosortaccount="p.accounting_code_sell";
    ...
    while ($i < min($num, $limit))
    {
		...
		if (! empty($conf->global->ACCOUNTING_SHOW_PROD_DESC))
		{
		...
		print '<td class="left">';
		if ($accounting_product_mode == 'ACCOUNTING_BUY') {
		    print length_accountg($obj->accounting_code_buy);
		    if ($obj->accounting_code_buy && empty($obj->aaid)) print ' '.img_warning($langs->trans("ValueNotIntoChartOfAccount"));
		}
		else
		{
		    print length_accountg($obj->accounting_code_sell);
		    if ($obj->accounting_code_sell && empty($obj->aaid)) print ' '.img_warning($langs->trans("ValueNotIntoChartOfAccount"));
		}
		print '</td>';
		...
		if ($accounting_product_mode == 'ACCOUNTING_BUY') {
			...
			$codesell=length_accountg($obj->accounting_code_buy);
			...
			$codesell=length_accountg($obj->accounting_code_sell);
