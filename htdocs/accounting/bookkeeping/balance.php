...
/**
 *  \file 		htdocs/accounting/bookkeeping/balance.php
 *  \ingroup 	Accounting
...
if (! empty($search_accounting_code_start)) {
	...
	$param .= '&amp;search_accounting_code_start=' . $search_accounting_code_start;
}
if (! empty($search_accounting_code_end)) {
	$filter['t.numero_compte<='] = $search_accounting_code_end;
	$param .= '&amp;search_accounting_code_end=' . $search_accounting_code_end;
}
...
if ($action != 'export_csv')
{
	...
	print_barre_liste($title_page, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $button, $result, $nbtotalofrecords, 'title_accounting', 0, '', '', $limit);
	...
	print $formaccounting->select_account($search_accounting_code_start, 'search_accounting_code_start', 1, array(), 1, 1, '');
	print ' ';
	print $langs->trans('to');
	print $formaccounting->select_account($search_accounting_code_end, 'search_accounting_code_end', 1, array(), 1, 1, '');
	print '</td>';
