...
$langs->loadLangs(array("accounting"));
...
$search_accounting_code_start = GETPOST('search_accounting_code_start', 'alpha');
if ($search_accounting_code_start == - 1) {
	$search_accounting_code_start = '';
}
$search_accounting_code_end = GETPOST('search_accounting_code_end', 'alpha');
if ($search_accounting_code_end == - 1) {
	$search_accounting_code_end = '';
}

$search_accounting_aux_code = GETPOST("search_accounting_aux_code");
$search_accounting_aux_code_start = GETPOST('search_accounting_aux_code_start', 'alpha');
if ($search_accounting_aux_code_start == - 1) {
	$search_accounting_aux_code_start = '';
}
$search_accounting_aux_code_end = GETPOST('search_accounting_aux_code_end', 'alpha');
if ($search_accounting_aux_code_end == - 1) {
	$search_accounting_aux_code_end = '';
}
...
if (! empty($search_accounting_code)) {
	$filter['t.numero_compte'] = $search_accounting_code;
	$param .= '&search_accounting_code=' . urlencode($search_accounting_code);
}
if (! empty($search_accounting_code_start)) {
	$filter['t.numero_compte>='] = $search_accounting_code_start;
	$param .= '&search_accounting_code_start=' . urlencode($search_accounting_code_start);
}
if (! empty($search_accounting_code_end)) {
	$filter['t.numero_compte<='] = $search_accounting_code_end;
	$param .= '&search_accounting_code_end=' . urlencode($search_accounting_code_end);
}
if (! empty($search_accounting_aux_code)) {
	$filter['t.subledger_account'] = $search_accounting_aux_code;
	$param .= '&search_accounting_aux_code=' . urlencode($search_accounting_aux_code);
}
if (! empty($search_accounting_aux_code_start)) {
	$filter['t.subledger_account>='] = $search_accounting_aux_code_start;
	$param .= '&search_accounting_aux_code_start=' . urlencode($search_accounting_aux_code_start);
}
if (! empty($search_accounting_aux_code_end)) {
	$filter['t.subledger_account<='] = $search_accounting_aux_code_end;
	$param .= '&search_accounting_aux_code_end=' . urlencode($search_accounting_aux_code_end);
}
...
if ($action == 'delbookkeepingyear') {
	...
	$year_array = $formaccounting->selectyear_accounting_bookkepping($delyear, 'delyear', 0, 'array');
...
print_barre_liste($title_page, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $button, $result, $nbtotalofrecords, 'title_accounting', 0, $groupby.$newcardbutton, '', $limit);
...
if (! empty($arrayfields['t.numero_compte']['checked']))
{
	...
	print '<div class="nowrap">';
	print $langs->trans('From').' ';
	print $formaccounting->select_account($search_accounting_code_start, 'search_accounting_code_start', 1, array (), 1, 1, 'maxwidth200');
	print '</div>';
	print '<div class="nowrap">';
	print $langs->trans('to').' ';
	print $formaccounting->select_account($search_accounting_code_end, 'search_accounting_code_end', 1, array (), 1, 1, 'maxwidth200');
	print '</div>';
	print '</td>';
}
// Subledger account
if (! empty($arrayfields['t.subledger_account']['checked']))
{
	...
	{
		print $formaccounting->select_auxaccount($search_accounting_aux_code_start, 'search_accounting_aux_code_start', 1);
	}
	else
	{
		print '<input type="text" name="search_accounting_aux_code_start" value="'.$search_accounting_aux_code_start.'">';
	}
	print '</div>';
	print '<div class="nowrap">';
	print $langs->trans('to').' ';
	...
	if (! empty($conf->global->ACCOUNTING_COMBO_FOR_AUX))
	{
		print $formaccounting->select_auxaccount($search_accounting_aux_code_end, 'search_accounting_aux_code_end', 1);
	}
	else
	{
		print '<input type="text" name="search_accounting_aux_code_end" value="'.$search_accounting_aux_code_end.'">';
	}
	print '</div>';
	print '</td>';
}
