...
/**
 * \file        htdocs/accounting/bookkeeping/thirdparty_lettering_customer.php
 * \ingroup     Accounting
...
$langs->loadLangs(array("compta","accounting"));
...
dol_syslog("/accounting/bookkeeping/thirdparty_lettering_customer.php", LOG_DEBUG);
$resql = $db->query($sql);
if (! $resql) {
	dol_print_error($db);
	exit();
...
if ($resql) {
	...
	//print '<a class="butAction" href="' . $_SERVER["PHP_SELF"] . '?socid=' . $object->id . '&action=autolettering">' . $langs->trans('AccountingAutoLettering') . '</a>';
	print "</form>";
	$db->free($resql);
} else {
	dol_print_error($db);
