...
/**
 * \file	    htdocs/accounting/class/accountingcategory.class.php
 * \ingroup     Accounting
...
class AccountingCategory // extends CommonObject
{
	...
	/**
	 * @var string Accounting code
	 */
	public $code;

	/**
     * @var string Accounting Category label
     */
    public $label;

    /**
	 * @var string Accounting range account
	 */
	...
	/**
	 * @var int Category type of accounting
	 */
	...
	public function getSumDebitCredit($cpt, $date_start, $date_end, $sens, $thirdparty_code = 'nofilter', $month = 0, $year = 0)
	{
		...
		if (is_array($cpt)) $sql.=", t.numero_compte as accounting_account";
		$sql .= " FROM " . MAIN_DB_PREFIX . "accounting_bookkeeping as t";
		...
		if ($resql)
		{
			...
			if ($num)
			{
				...
				if (is_array($cpt))
				{
					$this->sdcperaccount[$obj->accounting_account] = $this->sdc;
				}
			}
			return $num;
		} else {
