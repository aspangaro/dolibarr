...
$langs->loadLangs(array("bills","compta","accounting","other","productbatch"));
...
$hookmanager->initHooks(array('accountingcustomerlist'));
...
$sql.= " p.rowid as product_id, p.ref as product_ref, p.label as product_label, p.fk_product_type as type, p.accounting_code_sell as code_sell, p.tva_tx as tva_tx_prod,";
$sql.= " p.accounting_code_sell_intra as code_sell_intra, p.accounting_code_sell_export as code_sell_export,";
...
$sql.= " LEFT JOIN " . MAIN_DB_PREFIX . "accounting_account as aa ON p.accounting_code_sell = aa.account_number AND aa.fk_pcg_version = '" . $chartaccountcode."' AND aa.entity = " . $conf->entity;
$sql.= " LEFT JOIN " . MAIN_DB_PREFIX . "accounting_account as aa2 ON p.accounting_code_sell_intra = aa2.account_number AND aa2.fk_pcg_version = '" . $chartaccountcode."' AND aa2.entity = " . $conf->entity;
$sql.= " LEFT JOIN " . MAIN_DB_PREFIX . "accounting_account as aa3 ON p.accounting_code_sell_export = aa3.account_number AND aa3.fk_pcg_version = '" . $chartaccountcode."' AND aa3.entity = " . $conf->entity;
...
$sql .= " AND f.entity IN (" . getEntity('invoice', 0) . ")";    // We don't share object for accounting
...
if ($result) {
	...
	print_barre_liste($langs->trans("InvoiceLines"), $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num_lines, $nbtotalofrecords, 'title_accounting', 0, '', '', $limit);
