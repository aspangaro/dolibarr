...
/**
 * \file		htdocs/accounting/expensereport/index.php
 * \ingroup		Accounting
...
$langs->loadLangs(array("compta","bills","other","main","accounting"));
...
if ($action == 'clean' || $action == 'validatehistory')
{
	...
	dol_syslog("htdocs/accounting/customer/index.php fixaccountingcode", LOG_DEBUG);
...
if ($action == 'validatehistory') {
	...
	if ($db->type == 'pgsql') {
		...
		$sql1 .= " AND accnt.active = 1 AND t.accounting_code = accnt.account_number";
	...
	dol_syslog('htdocs/accounting/expensereport/index.php');
...
print load_fiche_titre($langs->trans("ExpenseReportsVentilation") . "&nbsp;" . $textprevyear . "&nbsp;" . $langs->trans("Year") . "&nbsp;" . $year_start . "&nbsp;" . $textnextyear, '', 'title_accounting');
...
dol_syslog('/accounting/expensereport/index.php:: sql=' . $sql);
$resql = $db->query($sql);
if ($resql) {
	...
	while ( $row = $db->fetch_row($resql)) {
		...
		if ($row[0] == 'tobind')
		{
			print $langs->trans("UseMenuToSetBindindManualy", DOL_URL_ROOT.'/accounting/expensereport/list.php?search_year='.$y, $langs->transnoentitiesnoconv("ToBind"));
		}
		else print $row[1];
		print '</td>';
    	for($i = 2; $i <= 12; $i ++) {
