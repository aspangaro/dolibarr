...
/**
 * \file 		htdocs/accounting/expensereport/lines.php
 * \ingroup 	Accounting
...
$langs->loadLangs(array("compta","bills","other","accounting","trips","productbatch"));
...
if (is_array($changeaccount) && count($changeaccount) > 0) {
	...
	if (! $error)
	{
		...
		dol_syslog('accounting/expensereport/lines.php::changeaccount sql= ' . $sql1);
...
dol_syslog('accounting/expensereport/lines.php::list');
$result = $db->query($sql);
...
if ($result) {
	...
	print_barre_liste($langs->trans("ExpenseReportLinesDone"), $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, '', $num_lines, $nbtotalofrecords, 'title_accounting', 0, '', '', $limit);
