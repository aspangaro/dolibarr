...
/**
 *  \file       htdocs/accounting/journal/bankjournal.php
 *  \ingroup    Accounting
...
$langs->loadLangs(array("companies","other","compta","banks","bills","donations","loan","accounting","trips","salaries","hrm","members"));
...
$sql .= " ba.courant, ba.ref as baref, ba.account_number, ba.fk_accounting_journal,";
...
$sql .= " u.accounting_code, u.rowid as userid, u.lastname as lastname, u.firstname as firstname, u.email as useremail, bu2.type as typeop_user,";
...
$sql .= " WHERE ba.fk_accounting_journal=" . $id_journal;
$sql .= ' AND b.amount != 0 AND ba.entity IN ('.getEntity('bank_account', 0).')';		// We don't share object for accounting
...
dol_syslog("accounting/journal/bankjournal.php", LOG_DEBUG);
$result = $db->query($sql);
if ($result) {
	...
	while ( $i < $num )
	{
		...
		// Set accounting code for bank
		$compta_bank = $obj->account_number;

		// Set accounting code for thirdparty
		...
		// Set accounting code for user
		$compta_user = (! empty($obj->accounting_code) ? $obj->accounting_code : $account_employee);
		...
		if (is_array($links) && count($links) > 0) {
			...
			foreach ($links as $key => $val) {
				...
				if ($links[$key]['type'] == 'payment') {
					...
					$sqlmid = 'SELECT cchgsoc.accounting_code';
					...
					dol_syslog("accounting/journal/bankjournal.php:: sqlmid=" . $sqlmid, LOG_DEBUG);
					$resultmid = $db->query($sqlmid);
					if ($resultmid) {
						$objmid = $db->fetch_object($resultmid);
						$tabtp[$obj->rowid][$objmid->accounting_code] += $obj->amount;
					}
					...
					$account_various = (! empty($paymentvariousstatic->accounting_code) ? $paymentvariousstatic->accounting_code : 'NotDefined');	// NotDefined is a reserved word
					...
					$sqlmid = 'SELECT pl.amount_capital, pl.amount_insurance, pl.amount_interest, l.accounting_account_capital, l.accounting_account_insurance, l.accounting_account_interest';
					...
					if ($resultmid) {
						$objmid = $db->fetch_object($resultmid);
						$tabtp[$obj->rowid][$objmid->accounting_account_capital] -= $objmid->amount_capital;
						$tabtp[$obj->rowid][$objmid->accounting_account_insurance] -= $objmid->amount_insurance;
						$tabtp[$obj->rowid][$objmid->accounting_account_interest] -= $objmid->amount_interest;
					}
		...
		/*if ($action == 'writebookkeeping')		// Make test now in such a case
		{
			...
			{
				$error++;
				setEventMessages($langs->trans('ErrorAccountingCodeOnBankAccountNotDefined', $obj->baref), null, 'errors');
			}
			...
			{
				$error++;
				setEventMessages($langs->trans('ErrorAccountingCodeOnThirdPartyNotDefined'), null, 'errors');
			}
		}*/
				...
				foreach ($tabtp[$key] as $k => $mt) {
					if ($mt)
					{
						...
						if ($tabtype[$key] == 'payment') {	// If payment is payment of customer invoice, we get ref of invoice
							...
							$bookkeeping->subledger_account = $tabuser[$key]['accounting_code'];
...
if (empty($action) || $action == 'view') {
	...
	$sql = 'SELECT COUNT(rowid) as nb FROM '.MAIN_DB_PREFIX.'bank_account WHERE fk_accounting_journal IS NULL AND clos=0';
	$resql = $db->query($sql);
	if ($resql)
	{
		...
		if ($obj->nb > 0)
		{
			...
			print ' : '.$langs->trans("AccountingAreaDescBank", 9, '<strong>'.$langs->transnoentitiesnoconv("MenuAccounting").'-'.$langs->transnoentitiesnoconv("MenuAccounting").'-'.$langs->transnoentitiesnoconv("Setup")."-".$langs->transnoentitiesnoconv("BankAccounts").'</strong>');
		}
	}
	...
	if (empty($conf->global->ACCOUNTING_ACCOUNT_CUSTOMER) || $conf->global->ACCOUNTING_ACCOUNT_CUSTOMER == '-1'
		...
		print ' : '.$langs->trans("AccountingAreaDescMisc", 4, '<strong>'.$langs->transnoentitiesnoconv("MenuAccounting").'-'.$langs->transnoentitiesnoconv("MenuAccounting").'-'.$langs->transnoentitiesnoconv("Setup")."-".$langs->transnoentitiesnoconv("MenuDefaultAccounts").'</strong>');
	}
