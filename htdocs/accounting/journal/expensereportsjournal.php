...
/**
 * \file		htdocs/accounting/journal/expensereportsjournal.php
 * \ingroup		Accounting
...
$langs->loadLangs(array("commercial", "compta","bills","other","accounting","trips","errors"));
...
$sql .= " u.rowid as uid, u.firstname, u.lastname, u.accounting_code as user_accounting_account,";
$sql .= " f.accounting_code, aa.rowid as fk_compte, aa.account_number as compte, aa.label as label_compte";
//$sql .= " ct.accounting_code_buy as account_tva";
...
$sql .= " AND er.entity IN (" . getEntity('expensereport', 0) . ")";  // We don't share object for accounting
...
dol_syslog('accounting/journal/expensereportsjournal.php', LOG_DEBUG);
$result = $db->query($sql);
if ($result) {
	...
	while ( $i < $num ) {
		...
		// Controls
		$compta_user = (! empty($obj->user_accounting_account)) ? $obj->user_accounting_account : $account_salary;
		$compta_fees = $obj->compte;

		$vatdata = getTaxesFromId($obj->tva_tx.($obj->vat_src_code?' ('.$obj->vat_src_code.')':''), $mysoc, $mysoc, 0);
		$compta_tva = (! empty($vatdata['accounting_code_buy']) ? $vatdata['accounting_code_buy'] : $account_vat);
		$compta_localtax1 = (! empty($vatdata['accounting_code_buy']) ? $vatdata['accounting_code_buy'] : $cpttva);
		$compta_localtax2 = (! empty($vatdata['accounting_code_buy']) ? $vatdata['accounting_code_buy'] : $cpttva);
		...
		$tabuser[$obj->rowid] = array (
				...
				'user_accounting_code' => $obj->user_accounting_account
		);
		...
		if (! $errorforline)
		{
			foreach ($tabttc[$key] as $k => $mt) {
				if ($mt) {
					...
					$bookkeeping->subledger_account = $tabuser[$key]['user_accounting_code'];
