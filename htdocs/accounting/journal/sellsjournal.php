...
$langs->loadLangs(array("commercial", "compta","bills","other","accounting","errors"));
...
$sql .= " p.rowid as pid, p.ref as pref, p.accounting_code_sell, aa.rowid as fk_compte, aa.account_number as compte, aa.label as label_compte";
...
$sql .= " AND f.entity IN (".getEntity('invoice', 0).')';	// We don't share object for accounting, we use source object sharing
...
if ($result) {
	...
	while ( $i < $num ) {
		...
		$compta_tva = (! empty($vatdata['accounting_code_sell']) ? $vatdata['accounting_code_sell'] : $cpttva);
		$compta_localtax1 = (! empty($vatdata['accounting_code_sell']) ? $vatdata['accounting_code_sell'] : $cpttva);
		$compta_localtax2 = (! empty($vatdata['accounting_code_sell']) ? $vatdata['accounting_code_sell'] : $cpttva);
