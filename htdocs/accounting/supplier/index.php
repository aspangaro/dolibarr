...
/**
 * \file		htdocs/accounting/supplier/index.php
 * \ingroup		Accounting
...
if ($action == 'validatehistory') {
	...
	if ($db->type == 'pgsql') {
		...
		$sql1 .= " AND accnt.active = 1 AND p.accounting_code_buy=accnt.account_number";
	...
	dol_syslog('htdocs/accounting/supplier/index.php');
...
print load_fiche_titre($langs->trans("SuppliersVentilation") . " " . $textprevyear . "&nbsp;" . $langs->trans("Year") . "&nbsp;" . $year_start . "&nbsp;" . $textnextyear, '', 'title_accounting');
...
$sql .= " AND ff.entity IN (" . getEntity('facture_fourn', 0) . ")";     // We don't share object for accounting
...
if ($resql) {
	...
	while ( $row = $db->fetch_row($resql)) {
		...
		if ($row[0] == 'tobind')
		{
			print $langs->trans("UseMenuToSetBindindManualy", DOL_URL_ROOT.'/accounting/supplier/list.php?search_year='.$y, $langs->transnoentitiesnoconv("ToBind"));
		}
		else print $row[1];
		print '</td>';
		for($i = 2; $i <= 12; $i ++) {
