...
if ($id > 0 || !empty($ref)) {
	...
	$canread = 0;
	if (!empty($user->hasRight('salaries', 'readall'))) {
		$canread = 1;
	}
	if (!empty($user->hasRight('salaries', 'read')) && $object->fk_user > 0 && in_array($object->fk_user, $childids)) {
		$canread = 1;
	}
	if (!$canread) {
		accessforbidden();
...
if ($id > 0) {
	...
	if ($action != 'edit') {
		...
		if (!empty($permissiontodelete) && empty($totalpaid)) {
